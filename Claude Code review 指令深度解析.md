---
name: claude-code-review-指令源码深度解析
description: 阅读 Claude Code 插件市场源码后整理的 /review 两个命令的内部工作机制
date: 2025-05-14
metadata:
  type: reference
---
...
## 十、实战案例：用 `/pr-review-toolkit:review-pr code` 评审一个工具模块

### 10.1 测试场景

在 `e:\代码库\todolist` 项目中，新建 `src/utils/math.js`，**故意埋 4 类典型坑**，看 review agent 能否抓出来。

**埋的坑：**

| 函数 | 坑 |
|------|-----|
| `percentage` | `toFixed` 返回字符串而非数字；`total=0` 时返回 `"NaN"` |
| `fetchNumbers` | 没检查 `res.ok`、没 try/catch、没验证返回数据结构 |
| `variance` | 局部 `let sum` 遮蔽了模块级 `sum` 函数；空数组 `NaN` |
| `max` / `min` | 空数组返回 `±Infinity`，没边界守护 |
...
参数 `code` 表示只跑 `code-reviewer` 一个 agent（其他维度跳过）。

### 10.3 执行过程

```
Step 1: 确定范围
        └─ git status 识别变更：math.js 是新文件

Step 2: 解析参数
        └─ code → 只用 code-reviewer

Step 3: 启动单 agent
        └─ Sonnet 级 code-reviewer，prompt 里指定：
           - 项目背景（Vite + React，无测试）
           - 关注点（正确性 / 错误处理 / 类型一致性 / 命名冲突）
           - 不要报告什么（缺测试、lint、风格 nit）
           - 输出格式（severity + 文件:行 + 修复方案）

Step 4: 等待结果（实测 32 秒，23K tokens）
```
...
|----------|------|-----------|
| 🔴 Critical | 3 | 全部命中预埋坑 |
| 🟡 Important | 2 | 1 个预埋坑 + 1 个额外发现（`clamp` 不校验边界） |
| 🔵 Suggestion | 1 | 额外发现：NaN 污染传播 |

**关键观察：**

1. ✅ **预埋的 4 类坑全部抓到**，没有遗漏
2. ✅ **额外发现 2 个我没想到的问题**（`clamp` 反向边界、`NaN` 跨函数传播）
3. ✅ **每个问题都标了 80-95 的置信度**，没有低质量假阳性
4. ✅ **修复建议直接给可粘贴代码**，不是空话
5. ✅ **没误报项目内部约定外的问题**（如缺测试、缺 JSDoc）
...
### 1. `percentage` returns a string, not a number — math.js:17-19 (confidence: 95)
toFixed(2) returns a String. Callers doing arithmetic (percentage(a,b) + 1)
get string concatenation ("50.001"), and chart/UI code expecting a number
will silently misbehave. Also explodes on total === 0 → returns "NaN".
...
格式包含：
- **严重等级**：Critical / Important / Suggestion
- **置信度**：每个 issue 标 0-100
- **文件:行号**：精确定位
- **可粘贴的修复代码**：直接抄就行
- **问题影响说明**：解释为什么是 bug
...
agent 未指出：
- `percentage` 没处理负数 `part`
- `fetchNumbers` 缺超时控制（AbortController）
- 整体缺 JSDoc 类型注释

但这些都属于 nitpick，按命令模板 Step 6 的过滤规则**应该被丢掉**，所以不算 bug。
...
- ✅ 新写的工具函数 / 模块自检
- ✅ 提交前的"第二双眼睛"
- ✅ Code review PR 前的预清洁
- ✅ 学习"应该考虑哪些边界"的训练

**该工具不适合：**
- ❌ 替代人工最终评审（业务逻辑判断仍需人）
- ❌ 评审超大 PR（单 agent 上下文有限）
- ❌ 没有 git 仓库的项目（依赖 `git diff`）

**最佳实践：**
1. **先 commit 干净的**，留 review 给增量改动
2. **参数明确**：`code` 比 `all` 快很多，针对性强
3. **prompt 加约束**：告诉 agent 项目背景和"不要报什么"
4. **修完复跑**：验证问题已解决
5. **配合 CLAUDE.md**：把项目规范写进去，agent 会按规范审
...
```
# 1. 写代码
vim src/utils/math.js

# 2. 自检（不需要 commit，工具看 git diff）
/pr-review-toolkit:review-pr code
...
git commit -m "feat: add math utility module"

# 7. 推送 + 建 PR
git push -u origin feature/math-utils
gh pr create

# 8. PR 上跑严格 review（自动评论到 GitHub）
/code-review
```

整套下来，**从写代码到 merge 都有 AI 把关**，质量门槛比传统流程高一截。